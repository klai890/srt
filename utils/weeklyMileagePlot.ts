 ...
 */
export function getMileagePlotData(
  af: Date,
  bf: Date,
  data: Array<ActivityWeek>,
  time_interval: number = ONE_WEEK,
): Array<MileagePoint> {
  ...
  for (
    var d: Date = new Date(af.valueOf() - time_interval);
    d < new Date(af);
  ...
  for (var d: Date = new Date(af); d < bf; d.setDate(d.getDate() + 1)) {
    ...
    var key: string = activityWeekKey(d);
    try {
      element = data.find((item) => item.week == monday.toLocaleDateString())[
        key
      ];
    } catch {
      element = 0;
